...
class BoardForm extends React.Component {
  static propTypes = {
    ...
    // dot notation if its exported as an object {}
  ...
  render() {
    ...
    return (
      <form className='col-6 offset-3 BoardForm'>
        ...
        {
          (editMode) ? (<button className="btn btn-warning" onClick={this.updateBoardEvent}>Update Board</button>) : (<button className="btn btn-secondary" onClick={this.saveBoardEvent}>Save Board</button>)
        }
      </form>
    );
  }
